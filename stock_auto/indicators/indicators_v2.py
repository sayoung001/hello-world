...
def calculate_indicators(df, market='US'):
    """
    보조지표 계산
    ...
    ★ 통화버그 수정 (분석 §3.1):
      기존은 market 인자 없이 호출돼 liquidity_score가 항상 'US' 기준이었다
      → KR 종목(원화 거래대금)이 거의 전부 포화(≈1.4). 이제 market을 끝까지 전달한다.
    """
    # [1] 전처리
    ...
    df['liquidity_score'] = _calc_liquidity_score(tv_ma5, market)  # ★ market 전달(버그 수정)
