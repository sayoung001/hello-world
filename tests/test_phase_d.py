...
def test_enriched_state_minimal():
    ...
    _test_result("builder_init_empty",
    ...
    _test_result("builder_pipeline_ctx",
                isinstance(ctx, PipelineContext),
                f"PipelineContext 생성 OK")
    _test_result("builder_market_state",
                ctx.market_state.btc_price == 95000
                and ctx.market_state.btc_level == 2,
                f"MarketState: BTC ${ctx.market_state.btc_price:,.0f}")
    _test_result("builder_signal_passed",
                ctx.signal.get("symbol") == "ETHUSDT",
                f"signal 전달 OK")
    _test_result("builder_empty_positions",
                len(ctx.positions) == 0,
                "포지션 없음")
    _test_result("builder_empty_vision",
                ctx.vision_features == {},
                "Vision 피처 없음 (정상)")
    _test_result("builder_empty_rag",
                ctx.rag_context == {},
                "RAG 피처 없음 (정상)")
...
def test_v9_hook_reflection_automation():
    ...
    _test_result("reflect_snapshot_saved",
                "ETHUSDT" in hook._entry_snapshots,
                f"진입 스냅샷 저장됨")
    ...
    _test_result("reflect_multi_trades",
...
def test_learning_loop():
    ...
    _test_result("loop_initial_advice",
...
def test_disable_flags():
    ...
    _test_result("disabled_risk_pipeline",
                hook_disabled._risk_pipeline is None,
                "리스크 파이프라인 비활성화")
    _test_result("disabled_brain",
