...
def test_embedder():
    ...
    try:
        ...
        test_result("embedder_single", len(vec) == 384,
                    f"차원: {len(vec)}")

        # 배치 임베딩
        texts = ["ETH 수렴 브레이크아웃", "BTC 하락 추세", "알트코인 펌프"]
        vecs = emb.embed_batch(texts)
        test_result("embedder_batch", len(vecs) == 3 and len(vecs[0]) == 384,
                    f"배치 크기: {len(vecs)}, 차원: {len(vecs[0])}")

        # 유사도 테스트
        sim = emb.similarity("BTC 상승", "비트코인 가격 올랐다")
        sim2 = emb.similarity("BTC 상승", "오늘 날씨가 좋다")
        test_result("embedder_similarity", sim > sim2,
                    f"관련 유사도: {sim:.4f} > 비관련: {sim2:.4f}")

        # 차원 수
        test_result("embedder_dimension", emb.dimension == 384,
                    f"dimension: {emb.dimension}")

    except Exception as e:
        test_result("embedder", False, str(e))
...
def test_vectorstore():
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            test_result("vectorstore_create", coll is not None,
                        f"컬렉션 생성 완료")
            ...
            test_result("vectorstore_add", cnt == 3,
                        f"문서 수: {cnt}")
            ...
            test_result("vectorstore_query", len(found_docs) >= 1,
            ...
            test_result("vectorstore_relevance",
                        first_meta.get("symbol") == "ETHUSDT",
                        f"최상위 결과: {first_meta.get('symbol')}")

            # health check
            health = store.health_check()
            test_result("vectorstore_health", health["status"] == "ok",
                        f"상태: {health}")

    except Exception as e:
        test_result("vectorstore", False, str(e))
...
def test_retriever():
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            test_result("retriever_search", len(results) >= 1,
                        f"결과 수: {len(results)}")

            if results:
                top = results[0]
                test_result("retriever_similarity",
                            top["similarity"] > 0.5,
                            f"최고 유사도: {top['similarity']}, final_score: {top['final_score']}")

                test_result("retriever_recency",
                            top.get("recency_weight", 0) > 0,
                            f"시간 가중치: {top.get('recency_weight')}")

    except Exception as e:
        test_result("retriever", False, str(e))
...
def test_context_builder():
    ...
    try:
        ...
        test_result("context_llm_text",
                    "유사 거래" in llm_text and "승률" in llm_text,
                    f"텍스트 길이: {len(llm_text)}자")

    except Exception as e:
        test_result("context_builder", False, str(e))
...
def test_news_ingester():
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            test_result("news_ingester_cycle", count >= 0,
                        f"수집 건수: {count}")

            stored = ingester.stored_count
            test_result("news_ingester_stored", stored == count,
                        f"저장 건수: {stored}")

    except Exception as e:
        test_result("news_ingester", False, str(e))
...
def test_chart_renderer():
    ...
    try:
        ...
        test_result("chart_render_bytes", len(img_bytes) > 1000,
                    f"이미지 크기: {len(img_bytes)} bytes")

        # PNG 헤더 검증
        test_result("chart_render_png",
        ...
        with tempfile.NamedTemporaryFile(suffix=".png", delete=False) as f:
            ...
            test_result("chart_render_file", size > 1000,
                        f"파일 크기: {size} bytes, 경로: {path}")
            os.unlink(path)

    except Exception as e:
        test_result("chart_renderer", False, str(e))
...
def test_vision_scorer():
    ...
    try:
        ...
        test_result("scorer_trend", features["vision_trend"] == 0.6,
                    f"trend: {features['vision_trend']}")
        test_result("scorer_conf", features["vision_pattern_conf"] == 0.75,
                    f"pattern_conf: {features['vision_pattern_conf']}")
        test_result("scorer_align", features["vision_breakout_align"] == 1.0,
                    f"breakout_align: {features['vision_breakout_align']}")
        test_result("scorer_risk", features["vision_risk_level"] == 0.2,
                    f"risk_level: {features['vision_risk_level']}")

        # 반대 방향 시그널
        features_opp = scorer.to_state_features(analysis, signal_direction="short")
        test_result("scorer_opposite", features_opp["vision_breakout_align"] == -1.0,
                    f"반대 align: {features_opp['vision_breakout_align']}")

        # confidence 조정
        adj = scorer.compute_confidence_adjustment(features)
        test_result("scorer_adjustment", adj > 0,
                    f"일치 시 조정: +{adj}")

        adj_opp = scorer.compute_confidence_adjustment(features_opp)
        test_result("scorer_adj_opposite", adj_opp < 0,
                    f"반대 시 조정: {adj_opp}")

        # 에러 케이스
        empty = scorer.to_state_features({"parse_error": True})
        test_result("scorer_empty", empty["vision_trend"] == 0.0,
                    "에러 시 기본값 반환")

    except Exception as e:
        test_result("vision_scorer", False, str(e))
...
def test_vision_cache():
    ...
    try:
        ...
        test_result("cache_put_get", result is not None and result["trend"] == 0.5,
                    f"캐시 히트: {result}")

        # 미존재 키
        result_miss = cache.get("ETHUSDT", 1000000)
        test_result("cache_miss", result_miss is None,
                    "존재하지 않는 키 → None")

        # TTL 만료
        time.sleep(2.5)
        result_expired = cache.get("BTCUSDT", 1000000)
        test_result("cache_ttl", result_expired is None,
                    "TTL 만료 후 → None")
        ...
        test_result("cache_lru", cache_lru.size == 2 and cache_lru.get("A", 100) is None,
                    f"LRU 정리 후 크기: {cache_lru.size}")

        # 통계
        stats = cache_lru.stats()
        test_result("cache_stats", "total_entries" in stats,
                    f"통계: {stats}")

    except Exception as e:
        test_result("vision_cache", False, str(e))
...
def test_max_tokens_fix():
    ...
    try:
        ...
        test_result("max_tokens_llm_call", default_max == 4096,
        ...
        test_result("max_tokens_llm_json", default_max2 == 4096,
                    f"llm_json max_tokens 기본값: {default_max2}")

    except Exception as e:
        test_result("max_tokens", False, str(e))
