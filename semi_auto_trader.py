...
class Executor:
    ...
    def open_position(self, sym, direction, qty, entry, sl, tp):
        ...
        try:
            ...
            # [8.2-FIX] SL 미설정 시 포지션 즉시 청산 (무보호 방지)
            if not ids['sl']:
    ...
    def close_position(self, sym, direction, qty, price, reason):
        ...
        try:
            # [8.2-FIX] 헤지모드: positionSide와 reduceOnly 충돌 → reduceOnly 제거
...
class ConvergenceTrader:
    ...
    def try_enter(self, signal):
        ...
        if entry <= 0: return  # [8.2-FIX] 비정상 시그널 방어
        ...
        if tp_dist_orig <= 0: return  # [8.2-FIX] TP 거리 0 방어
        ...
        try:
            ...
            for pos in self.positions:
                ...
                if key not in ex_keys:
                    ...
                    removed.append(pos)
                    self._alerted_orders.discard(f"{pos.symbol}_{pos.direction}")
                    # [8.2-FIX] 잔여 주문 정리 (TP or SL 남아있을 수 있음)
                    try:
    ...
    def _load_positions(self):
        ...
        for try_path in [self.cfg["POSITIONS_FILE"], self.cfg["POSITIONS_FILE"] + '.bak']:
            ...
            try:
                ...
                continue
        self.positions = []
        if os.path.exists(self.cfg["POSITIONS_FILE"]):
